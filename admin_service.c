...
void show_admin_dashboard(void)
{
    ...
    printf("3. 患者总数：%d\n", patient_count);
    printf("4. 待诊患者数：%d\n", pending_count);
    printf("5. 已缴费待取药患者数：%d\n", paid_count);
    printf("6. 已完成患者数：%d\n", completed_count);
    printf("7. 当前低库存药品数量：%d\n", low_stock_count);
    printf("8. 当前待发药患者数量：%d\n", waiting_dispense_count);
    printf("9. 当前已占用床位数量：%d\n", occupied_bed_count);
    printf("10. 当前空闲床位数量：%d\n", free_bed_count);
    printf("======================================================\n");
...
void show_infectious_disease_alerts(void)
{
    ...
    if (g_patient_list != NULL && g_patient_list->next != NULL)
    {
        ...
        while (patient_curr != NULL)
        {
            ...
            // 检查是否有症状描述
            if (patient_curr->symptom != NULL && strlen(patient_curr->symptom) > 0)
            {
                symptomatic_count++;
                // 简单基于症状描述进行判断
