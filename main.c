...
static void handle_internal_mark_missed();
...
static void handle_patient_self_recharge();
...
static void handle_show_bed_occupancy();
static void handle_query_bed_by_id();
static void handle_bed_deposit_warnings();
...
printf("  [8] 查看药师值班状态\n");
        ...
        switch (get_safe_int("👉 请输入操作编号: "))
        {
            ...
            case 8:
                system("cls");
                show_all_pharmacists_with_duty_status();
                system("pause");
                break;
            case 9:
                system("cls");
                handle_admin_update_pharmacist_duty();
                system("pause");
                break;
            case 10:
                system("cls");
...
static void handle_admin_register_account()
{
    ...
    while (1)
    {
        switch (step)
        {
            ...
            case 3: // 输入性别
                get_safe_string("请输入性别 (1=男, 2=女): ", input_buffer, 100);
            ...
            case 5: // 输入医生科室（仅医生角色）
                ...
                if (!is_blank_string(input_buffer))
                {
                    ...
                    {
                        printf("[WARN] 医生链表尚未初始化，无法新增医生账号。\n");
                        return;
                    }
                    if (find_doctor_by_id(g_doctor_list, username) != NULL)
                    {
                        printf("[WARN] 已存在同编号医生实体，无法重复新增。\n");
                        return;
                    }
...
static void handle_admin_update_account()
{
    ...
    while (1)
    {
        ...
        {
            case 0: // 输入要修改的账号
                get_safe_string("请输入要修改的账号: ", input_buffer, 100);
                
                if (strcmp(input_buffer, "B") == 0 || strcmp(input_buffer, "b") == 0)
                {
                    printf("当前已是第一项，无法再返回\n");
                    continue;
                }
                else if (strcmp(input_buffer, "Q") == 0 || strcmp(input_buffer, "q") == 0)
                {
                    printf("提示：已取消修改员工资料操作。\n");
                    return;
                }
            ...
            case 1: // 新真实姓名
                get_safe_string("请输入新真实姓名（直接回车表示不修改）: ", input_buffer, 100);
            ...
            case 2: // 新密码
                get_safe_string("请输入新密码（直接回车表示不修改）: ", input_buffer, 100);
            ...
            case 3: // 角色修改
                ...
                {
                    target_role = ROLE_NURSE;
                }
                else if (role_choice == 3)
                {
                    target_role = ROLE_DOCTOR;
                }
                else if (role_choice == 4)
                {
                    target_role = ROLE_PHARMACIST;
                }
                else
                {
                    printf("[WARN] 无效的角色编号，请重新输入。\n");
                    continue;
                }
            ...
            case 4: // 医生科室
                ...
                {
                    printf("当前科室：%s\n", doctor->department);
                    get_safe_string("请输入新科室（直接回车表示不修改）: ", input_buffer, 100);
                }
                else
                {
                    get_safe_string("请输入医生所属科室: ", input_buffer, 100);
            ...
            case 5: // 摘要确认
                if (hasChanges == 0)
                {
                    printf("\n[WARN] 提示：您输入的信息与原信息完全一致，未做任何实际修改。\n");
                    return;
                }
                ...
                if (!is_blank_string(new_real_name))
                {
                    printf("真实姓名：%s -> %s\n", account->real_name, new_real_name);
                }
                else
                {
                    printf("真实姓名：%s（不修改）\n", account->real_name);
                ...
                printf("========================================\n");
                get_safe_string("确认提交修改？（Y=确认，N=取消，B=返回上一步）: ", input_buffer, 100);
            ...
            case 6: // 提交修改
                ...
                if (account == NULL)
                {
                    printf("[WARN] 账号更新后未能重新定位账号节点，请检查数据。\n");
                    return;
                }
                ...
                else if (account->role == ROLE_DOCTOR)
                {
                    ...
                    if (doctor == NULL)
                    {
                        if (is_blank_string(department))
                        {
                            printf("[WARN] 检测到缺失医生实体，但当前未提供科室，无法自动补建。\n");
                            return;
                        }
                        ...
                        if (doctor == NULL)
                        {
                            printf("[WARN] 医生实体补建失败，请手动处理。\n");
                            return;
                        }
                        insert_doctor_tail(g_doctor_list, doctor);
                        printf("[OK] 已补建缺失的医生实体。\n");
                    }
                    else
                    {
...
static void handle_admin_update_pharmacist_duty()
{
    ...
    while (1)
    {
        ...
        {
            case 0: // 第1步：输入药师账号
                get_safe_string("请输入药师账号: ", input_buffer, 100);
                ...
                if (account == NULL)
                {
                    printf("[WARN] 未找到对应药师账号，请重新输入。\n");
                    continue;
                }
                
                if (account->role != ROLE_PHARMACIST)
                {
                    printf("[WARN] 该账号不是药师角色，请重新输入。\n");
                    continue;
                }
...
back_to_decision:
    ...
    {
        printf("------------------------------------------\n");
        printf("[TIP] 提示：建议根据检查报告结果进行\"开药\"、\"住院\"或\"结束就诊\"\n");
    }
    else
    {
        printf("------------------------------------------\n");
    ...
    else if (decision == 4)
    {
        ...
        printf("\n[OK] 已为患者开具住院通知，请通知患者前往护士站办理住院登记！\n");
    ...
    if (decision == 5)
    {
        ...
        else
        {
            ...
            printf("\n[UPD] [过号顺延] 第 %d 次叫号未到，已顺延至 3 位之后。(满 3 次将作废)\n", patient->call_count);
        }
    ...
    if (decision == 2)
    {
        ...
        while (1)
        {
            ...
            if (strcmp(med_id, "0") == 0)
            {
                ...
                {
                    printf("[WARN] “开药”决策至少需要开出一种药品，当前不能空单结束！\n");
                    continue;
                }
                break;
            }
            ...
            if (quantity <= 0)
            {
                printf("[WARN] 开药数量必须大于 0！\n");
                continue;
            }

            if (quantity > med->stock)
            {
                printf("[WARN] 库存不足！当前库存为 %d，无法开具 %d。\n", med->stock, quantity);
                continue;
            }
            ...
            printf("[OK] 开药成功：%s x %d\n", med->name, quantity);
        }

        // 检查临时处方链表是否为空
        if (temp_prescription_list == NULL)
        {
            printf("\n[WARN] 本次接诊未追加任何处方药品，请确认是否符合医嘱预期。\n");
        }
    }
    else if (decision == 3)
    {
        ...
        while (1)
        {
            ...
            if (strcmp(item_id, "0") == 0)
            {
                ...
                {
                    printf("[WARN] “开检查”决策至少需要开出一个检查项目，当前不能空单结束！\n");
                    continue;
                }
                break;
            }
        ...
        if (temp_check_record_list == NULL)
        {
            printf("\n[WARN] 本次接诊未追加任何检查项目，请确认是否符合医嘱预期。\n");
        }
        consult_success = doctor_consult_patient(
            g_current_doctor->id,
            patient->id,
...
static void handle_query_check_records()
{
    ...
    printf("\n================ 查询检查记录 ================\n");
    printf("支持格式：患者编号（例如 P-1001） | 检查记录编号（例如 CR-001）\n");
    printf("提示：输入 Q 取消该操作返回上一级菜单\n\n");
    ...
    while (1)
    {
        ...
        if (strncmp(query_str, "P-", 2) == 0)
        {
            ...
            {
                printf("[WARN] 患者编号格式不合法（应为 P-4位数字，如 P-1001），请重新输入！\n");
                continue;
            }
        ...
        else if (strncmp(query_str, "CRK-", 4) == 0)
        {
            ...
            if (!valid)
            {
                printf("[WARN] 检查记录编号格式不合法（应为 CRK-4位数字，如 CRK-0001），请重新输入！\n");
                continue;
            }
        ...
        else if (strncmp(query_str, "CR-", 3) == 0)
        {
            ...
            if (!valid)
            {
                printf("[WARN] 检查记录编号格式不合法（应为 CR-3位数字，如 CR-001），请重新输入！\n");
                continue;
            }
            show_check_record_by_id(query_str);
            break;
        ...
        {
            printf("[WARN] 编号格式不合法（患者 P-xxxx / 记录 CR-xxx / CRK-xxxx），请重新输入！\n");
            continue;
        }
        break;
    }
...
static void handle_pharmacy_dispense()
{
    ...
    while (1)
    {
        ...
        {
            printf("[WARN] 患者编号格式不合法，正确格式为 P-XXXX，例如：P-1001，请重新输入！\n");
            printf("提示：输入 'B' 返回上一步，输入 'Q' 退出当前操作\n");
        }
    }
...
static void handle_internal_patient_register()
{
    ...
    while (1)
    {
        ...
        if (my_strcasecmp(name, "B") == 0)
        {
            printf("当前已是第一步，已返回上一级菜单\n");
            system("pause");
            return;
        }
    ...
    while (1)
    {
        get_safe_string("请输入患者性别 (1=男, 2=女): ", gender_str, 8);
        
        if (my_strcasecmp(gender_str, "Q") == 0)
        {
            printf("操作取消！\n");
            system("pause");
            return;
        ...
        if (my_strcasecmp(gender_str, "B") == 0)
        {
            goto input_age;
        }
        
        if (strcmp(gender_str, "1") == 0) {
            strcpy(gender_str, "男");
            break;
        } else if (strcmp(gender_str, "2") == 0) {
            strcpy(gender_str, "女");
            break;
        }
        printf("[WARN] 性别输入无效，请输入 1 或 2！\n");
    }
    
    input_id_card:
    while (1)
    {
        get_sensitive_string_with_toggle("请输入身份证号（Tab显/隐，Enter确认）：", id_card, MAX_ID_LEN);
        
        if (my_strcasecmp(id_card, "Q") == 0)
        {
            printf("操作取消！\n");
            system("pause");
            return;
    ...
    if (my_strcasecmp(target_dept, "Q") == 0)
    {
        printf("操作取消！\n");
        system("pause");
        return;
    ...
    if (my_strcasecmp(target_dept, "B") == 0)
    {
        goto input_symptom;
    }
    ...
    {
        char masked_id[19] = "";
        mask_id_card(id_card, masked_id);
        ...
        printf("年龄：%d\n", new_patient->age);
        printf("身份证号：%s\n", masked_id);
        printf("原选择科室：%s\n", strlen(target_dept) > 0 ? target_dept : "未指定");
        printf("实际就诊科室：%s\n", new_patient->target_dept);
        printf("当前状态：%s\n", get_patient_status_text(new_patient->status));
        printf("===============================================\n");
    }
    else
    {
...
static void handle_internal_appointment_register()
{
    ...
    if (my_strcasecmp(appointment_date, "Q") == 0)
    {
        printf("操作取消！\n");
        system("pause");
        return;
    ...
    if (my_strcasecmp(appointment_date, "B") == 0)
    {
    ...
    {
        printf("[WARN] 预约日期不能为空，请重新输入！\n");
        goto input_date;
    }
    
    if (!is_valid_date_string(appointment_date))
    {
        printf("[WARN] 预约日期格式非法，请使用 YYYY-MM-DD 格式重新输入！\n");
        goto input_date;
    }
    ...
    if (my_strcasecmp(slot_choice, "Q") == 0)
    {
        printf("操作取消！\n");
        system("pause");
        return;
    ...
    if (my_strcasecmp(slot_choice, "B") == 0)
    {
        goto input_date;
    }
    
    if (strcmp(slot_choice, "1") == 0) {
        strcpy(appointment_slot, "上午");
    } else if (strcmp(slot_choice, "2") == 0) {
        strcpy(appointment_slot, "下午");
    } else {
        printf("[WARN] 无效的时段选择，请输入 1 或 2！\n");
        goto input_slot;
    }
    ...
    if (my_strcasecmp(appoint_dept, "Q") == 0)
    {
        printf("操作取消！\n");
        system("pause");
        return;
    ...
    if (my_strcasecmp(appoint_dept, "B") == 0)
    {
        goto input_slot;
    }
    ...
    if (my_strcasecmp(appoint_doctor, "Q") == 0)
    {
        printf("操作取消！\n");
        system("pause");
        return;
    ...
    if (my_strcasecmp(appoint_doctor, "B") == 0)
    {
        goto input_dept;
    }
...
static void handle_internal_appointment_cancel()
{
    ...
    while (1)
    {
        ...
        if (my_strcasecmp(appointment_id, "Q") == 0)
        {
            printf("操作取消！\n");
            system("pause");
            return;
        ...
        if (my_strcasecmp(appointment_id, "B") == 0)
        {
            printf("当前已是第一步，已返回上一级菜单\n");
            system("pause");
            return;
...
static void handle_patient_self_balance_query()
{
    ...
    input_balance_patient_id:
    while (1)
    {
        get_safe_string("请输入患者编号: ", patient_id, MAX_ID_LEN);
        if (my_strcasecmp(patient_id, "Q") == 0) { printf("操作取消！\n"); return; }
        if (my_strcasecmp(patient_id, "B") == 0)
        {
            printf("当前已是第一步，已返回上一级菜单\n");
            system("pause");
    ...
    input_balance_id_card:
    while (1)
    {
        get_sensitive_string_with_toggle("请输入身份证号（Tab显/隐，Enter确认）：", id_card, MAX_ID_LEN);
        if (my_strcasecmp(id_card, "Q") == 0) { printf("操作取消！\n"); return; }
        if (my_strcasecmp(id_card, "B") == 0)
        {
            goto input_balance_patient_id;
        }
...
static void handle_patient_self_recharge()
{
    char patient_id[MAX_ID_LEN];
    char id_card[MAX_ID_LEN];
    char amount_str[MAX_SYMPTOM_LEN];
    double amount = 0.0;
    PatientNode* patient = NULL;
    ...
    while (1)
    {
        ...
        if (!validate_patient_id(patient_id))
        {
            printf("患者编号格式不合法，正确格式为 P-XXXX，请重新输入：");
            get_safe_string("", patient_id, MAX_ID_LEN);
            continue;
        }
    ...
    input_rc_amount:
    printf("\n请输入充值金额（输入 B 返回，输入 Q 取消）：");
    get_safe_string("", amount_str, MAX_SYMPTOM_LEN);
    if (strcmp(amount_str, "Q") == 0 || strcmp(amount_str, "q") == 0)
    {
        printf("提示：已返回菜单页。\n");
        return;
    }
    if (strcmp(amount_str, "B") == 0 || strcmp(amount_str, "b") == 0)
        goto input_rc_id_card;
    ...
    while (1)
    {
        ...
        {
            printf("充值金额不能为空，请重新输入：");
            get_safe_string("", amount_str, MAX_SYMPTOM_LEN);
            continue;
        }
        if (!parse_double_strict(amount_str, &amount))
        {
            printf("请输入有效的数字金额，请重新输入：");
            get_safe_string("", amount_str, MAX_SYMPTOM_LEN);
            continue;
        }
        if (amount <= 0)
        {
            printf("充值金额必须大于0，请重新输入：");
            get_safe_string("", amount_str, MAX_SYMPTOM_LEN);
            continue;
        }
        break;
    ...
    patient_recharge_balance(patient_id, amount);
    system("pause");
}
...
static void handle_patient_self_appointment_register()
{
    ...
    {
        printf("\n[WARN] 您已存在该时段的有效预约（预约号：%s），不能重复预约！\n", existing_apt_id);
        printf("按回车键继续...\n");
        getchar();
        return;
    }
    ...
    if (is_current_emergency != 1)
    {
        // [HIS] 普通门诊/专家号：严格执行预付费拦截机制
        if (patient->balance < reg_fee)
        {
            printf("\n[ERROR] 余额不足！需预先扣除挂号费 %.2f 元。\n", reg_fee);
    ...
    if (new_appt != NULL)
    {
        if (is_current_emergency == 1)
        {
            ...
            if (patient->balance < 0)
            {
                printf("[WARN] 提示：挂号费 %.2f 元已挂账，当前账户欠费 %.2f 元，请家属后续补缴。\n", reg_fee, -patient->balance);
            }
            else
            {
                printf("[FEE] 已扣除急诊挂号费：%.2f 元，当前余额：%.2f 元。\n", reg_fee, patient->balance);
            }
        }
        else
        {
            patient->balance -= reg_fee;
            printf("\n[OK] 扣费成功！本次挂号费：%.2f 元，账户余额：%.2f 元。\n", reg_fee, patient->balance);
        }
...
static void handle_patient_self_first_visit()
{
    ...
    char target_dept[MAX_NAME_LEN] = "";
    char gender_str[8] = "";
    PatientNode* existing_patient = NULL;
...
input_gender:
    while (1)
    {
        get_safe_string("请输入您的性别 (1=男, 2=女): ", gender_str, 8);
...
input_symptom:
    while (1)
    {
        ...
        if (my_strcasecmp(symptom_temp, "Q") == 0)
        {
            printf("操作取消！\n");
            system("pause");
            return;
        ...
        if (my_strcasecmp(symptom_temp, "B") == 0)
        {
            goto input_gender;
        }
        
        strncpy(symptom, symptom_temp, MAX_SYMPTOM_LEN - 1);
        symptom[MAX_SYMPTOM_LEN - 1] = '\0';
        break;
    }
    
    strcpy(target_dept, "普通内科");
...
static void handle_patient_self_registration()
{
    ...
    if (strcmp(recommended_dept, "急诊科") == 0 || is_night)
    {
        ...
        {
            printf("[ALERT] 症状符合急诊特征，已为您开启绿色生命通道！\n");
        }
        // 锁定科室为急诊科
        strcpy(appoint_dept, "急诊科");
        printf("[WARN] 已自动锁定科室为：急诊科\n");
    }
    else
    {
    ...
    // ==========================================
    {
        ...
        {
            printf("\n[WARN] 您已存在该时段的有效预约（预约号：%s），不能重复挂号/预约！\n", existing_appointment_id);
            printf("请选择其他时段。\n");
            goto input_slot;
        }
    }
    ...
    if (new_appt != NULL )
    {
        ...
        if (!queue_walk_in_registration(new_appt->appointment_id))
        {
            ...
            patient->symptom[MAX_SYMPTOM_LEN - 1] = '\0';
            patient->is_emergency = old_is_emergency;
            printf("\n[ERROR] 挂号后分诊失败，系统已自动撤销本次挂号并退回 %.2f 元。\n", reg_fee);
            system("pause");
            return;
        }
        ...
        printf("\n[OK] 现场挂号与分诊全部完成！请关注叫号屏幕，前往对应科室候诊。\n" );
    }
    else
    {
        strncpy(patient->symptom, old_symptom, MAX_SYMPTOM_LEN - 1);
...
static void handle_patient_self_appointment_cancel()
{
    ...
    while (1)
    {
        ...
        AppointmentNode* apt = find_appointment_by_id(g_appointment_list, appointment_id);
        if (apt == NULL)
        {
            printf("[WARN] 未找到对应预约记录，请重新输入！\n");
            continue;
        }
        
        if (strcmp(apt->patient_id, patient_id) != 0)
        {
            printf("[WARN] 无权取消他人预约，请重新输入！\n");
            continue;
        }
        break;
    }
...
static void handle_patient_archive_update()
{
    ...
    {
        if (strcmp(temp_input, patient->name) != 0)
        {
    ...
    if (strlen(temp_input) > 0)
    {
        if (strcmp(temp_input, patient->symptom) != 0)
        {
    ...
    if (strlen(temp_input) > 0)
    {
        if (strcmp(temp_input, patient->target_dept) != 0)
        {
    ...
    input_id_card:
    printf("当前身份证号：%s\n", patient->id_card);
    get_sensitive_string_with_toggle("请输入新身份证号（Tab显/隐，Enter确认）：", temp_input, MAX_ID_LEN);
    if (strcmp(temp_input, "Q") == 0 || strcmp(temp_input, "q") == 0)
    {
        printf("提示：已返回菜单页。\n");
        return;
    ...
    while (1)
    {
        ...
        if (strcmp(temp_input, patient->id_card) != 0)
        {
    ...
    while (strlen(temp_input) > 0)
    {
        ...
        if (new_balance != patient->balance)
        {
            balance = new_balance;
            has_modified = 1;
        }
    ...
    while (running)
    {
        ...
        switch (get_safe_int("👉 请输入操作编号: "))
        {
            ...
            case 4:
                ...
                if (my_strcasecmp(patient_id, "Q") == 0 || my_strcasecmp(patient_id, "B") == 0) break;
                ...
                if (my_strcasecmp(id_card, "Q") == 0 || my_strcasecmp(id_card, "B") == 0) break;
            ...
            case 13:
                ...
                while (1)
                {
                    ...
                    {
                        goto input_case13_id_card;
                    }
                    
                    if (parse_double_strict(input, &payment_amount) && payment_amount >= 0)
                    {
                        break;
                    }
                    else
                    {
                        printf("[WARN] 请输入有效的金额（数字）！\n");
                    }
                }
                
                if (my_strcasecmp(input, "Q") == 0 || my_strcasecmp(input, "B") == 0) break;
                
                if (payment_amount < debt_patient->emergency_debt)
                {
                    printf("\n[WARN] 实际缴费金额（%.2f 元）小于欠费金额（%.2f 元），缴费失败！\n", payment_amount, debt_patient->emergency_debt);
                    printf("请缴纳足额欠款后再来办理！\n");
                    system("pause");
                    break;
                }
                
                debt_patient->emergency_debt = 0.0;
                debt_patient->is_blacklisted = 0;
                debt_patient->status = STATUS_COMPLETED;
                
                printf("\n[OK] 缴费成功！您已移出黑名单，恢复正常就医权限。\n");
                printf("实际缴费金额: %.2f 元\n", payment_amount);
                printf("==========================================\n");
...
// 测试查找引擎
        ...
        if (search_result != NULL) {
            printf("[INFO] 雷达搜索成功！找到目标：患者姓名 [%s]，当前余额 [%.2f]\n\n", search_result->name, search_result->balance);
        } else {
            printf("[ERROR] 雷达搜索失败，查无此人！\n\n");
        }
...
"M-031", "复方甘草口服溶液", "甘草合剂", "用于一般性及化痰性咳嗽的对症治疗", "12.50", "800", "1", "2026-10-30"
        ...
        printf("[OK] 批量测试数据注入完毕！(包含150患者, 40住院, 120医生, 30药品, 50病床)\n");
        
        // 8. 模拟黑名单患者触发安全预警（让alerts.txt有数据）
        printf("\n[NOTE] 正在模拟黑名单患者行为，生成安全预警数据...\n");
        ...
        if (blacklisted_patient3 != NULL) {
            char alert_msg[256];
            snprintf(alert_msg, sizeof(alert_msg), 
                "[ALERT] 急诊强制放行：患者 %s（黑名单人员）突发危重急症，已强制放行，请相关部门跟进！", 
    ...
    while (1)
    {
        ...
        case 3:
            ...
            printf("  [4] 单人病房\n");
            {
                ...
                if (!is_valid_num)
                {
                    printf("\n[WARN] 无效输入，请输入 1-4，或输入 B 返回，Q 退出。\n\n");
                    continue;
                }
        ...
        case 4:
            ...
            printf("[WARN] 无效选择，请输入 Y、N、B 或 Q。\n");
            continue;
        }
    }
}
...
static void handle_admin_update_ward_bed()
{
    ...
    while (1)
    {
        ...
        case 0:
            ...
            if (my_strcasecmp(bed_id, "B") == 0 || my_strcasecmp(bed_id, "Q") == 0)
            {
                printf("\n已退出修改床位信息操作。\n");
                return;
            }
            ...
            printf("病房编号：%s\n", target->room_id);
            printf("床位编号：%s\n", target->bed_id);
            printf("所属科室：%s\n", strlen(target->dept) == 0 ? "无" : target->dept);
            printf("病房类型：");
        ...
        case 1:
            printf("\n请输入新病房编号（当前：%s，直接回车不修改）：\n", old_room_id);
            get_safe_string("新病房编号：", new_room_id, MAX_ID_LEN);
            if (my_strcasecmp(new_room_id, "Q") == 0)
            {
                printf("\n已退出修改床位信息操作。\n");
                return;
            }
            if (my_strcasecmp(new_room_id, "B") == 0)
            {
                printf("\n已返回床位资源管理菜单。\n");
                return;
            }
        ...
        case 2:
            printf("\n请输入新所属科室（当前：%s，直接回车不修改）：\n", strlen(old_dept) == 0 ? "无" : old_dept);
            get_safe_string("新所属科室：", new_dept, MAX_NAME_LEN);
            if (my_strcasecmp(new_dept, "Q") == 0)
            {
                printf("\n已退出修改床位信息操作。\n");
                return;
            }
            if (my_strcasecmp(new_dept, "B") == 0)
            {
                step = 1;
                continue;
            }
        ...
        case 3:
            ...
            get_safe_string("请输入选择：", input_buffer, sizeof(input_buffer));
            if (my_strcasecmp(input_buffer, "Q") == 0)
            {
                printf("\n已退出修改床位信息操作。\n");
                return;
            }
            if (my_strcasecmp(input_buffer, "B") == 0)
            {
                step = 2;
                continue;
            }

            new_ward_type = atoi(input_buffer);
            if (new_ward_type < 0 || new_ward_type > 4)
            {
                printf("\n[WARN] 非法输入！请重新输入0-4之间的纯数字。\n");
                continue;
            }
            step = 4;
            break;
        ...
        case 4:
            ...
            printf("\n================ 修改确认 ================\n");
            printf("床位编号：%s\n", bed_id);
            printf("病房编号：%s -> %s\n", strlen(old_room_id) == 0 ? "无" : old_room_id, strlen(new_room_id) == 0 ? "无" : new_room_id);
            printf("所属科室：%s -> %s\n", strlen(old_dept) == 0 ? "无" : old_dept, strlen(new_dept) == 0 ? "无" : new_dept);
            printf("病房类型：");
...
static void handle_admin_delete_ward_bed()
{
    ...
    while (1)
    {
        ...
        if (target->is_occupied)
        {
            printf("\n[WARN] 该床位正在被患者占用，不能删除/停用。\n");
            return;
        }

        while (1)
        {
            printf("\n是否确认删除/停用该床位？Y=确认，N=取消，B=返回上一步\n");
            confirm = get_single_char("");
    ...
    while (running)
    {
        ...
        switch (choice)
        {
            ...
            case 2:
                ...
                break;
            case 3:
                handle_show_bed_occupancy();
                system("pause");
                break;
            case 4:
                handle_query_bed_by_id();
                system("pause");
                break;
            case 5:
...
enter_patient:
    while (1)
    {
        ...
        if (patient == NULL)
        {
            printf("[WARN] 未找到该患者，请重新输入患者编号或先建档\n");
            continue;
        }
        ...
        {
            printf("[WARN] 该患者当前已住院，不能重复登记，请重新输入患者编号\n");
            continue;
        }

        if (patient->status != STATUS_NEED_HOSPITALIZE)
        {
            printf("[WARN] 该患者未被医生开具住院通知，不能办理住院登记！\n");
...
enter_days:
    while (1)
    {
        ...
        {
            printf("[WARN] 预计住院天数必须是正整数，请重新输入\n");
            continue;
        }
...
enter_deposit:
    while (1)
    {
        ...
        {
            printf("[WARN] 初始押金必须是合法数字，请重新输入\n");
            continue;
        }
        if (temp_deposit <= 0)
        {
            printf("[WARN] 初始押金必须大于0，请重新输入\n");
            continue;
        }
...
enter_patient_bed:
    ...
    while (1)
    {
        ...
        if (target_bed == NULL)
        {
            printf("[WARN] 床位不存在，请重新输入\n");
            continue;
        }
        if (target_bed->is_occupied)
        {
            printf("[WARN] 该床位已被占用，请重新输入\n");
            continue;
        }
...
static void handle_query_bed_by_id()
{
    ...
    while (1)
    {
        get_safe_string("请输入床位编号（输入B返回上一级）: ", bed_id, MAX_ID_LEN);
        if (my_strcasecmp(bed_id, "b") == 0) return;
        if (is_blank_string(bed_id))
        {
            printf("[WARN] 床位编号不能为空，请重新输入\n");
            continue;
    ...
    WardNode* ward = find_ward_by_id(g_ward_list, bed_id);
    if (ward == NULL)
    {
        printf("[WARN] 未找到床位编号为 %s 的床位\n", bed_id);
        return;
    }
    ...
    printf("病房编号: %s\n", ward->room_id);
    printf("床位编号: %s\n", ward->bed_id);
    printf("病房类型: %s\n", ward->ward_type == WARD_ICU ? "ICU" :
        (ward->ward_type == WARD_ISOLATION ? "隔离病房" :
        (ward->ward_type == WARD_SINGLE ? "单人病房" : "普通病房")));
    printf("所属科室: %s\n", ward->dept);
    printf("床位状态: %s\n", ward->is_occupied ? "占用" : "空闲");

    if (ward->is_occupied)
    {
        printf("患者编号: %s\n", ward->patient_id);
        PatientNode* patient = find_patient_by_id(g_patient_list, ward->patient_id);
        if (patient != NULL)
        {
            printf("患者姓名: %s\n", patient->name);
        }
...
static void handle_bed_deposit_warnings()
{
    ...
    int has_bed_warning = 0;
    WardNode* ward_curr = NULL;
    WardNode* bed = NULL;
    InpatientRecord* curr = NULL;
    PatientNode* patient = NULL;
    int warning_count = 0;
    const char* recommend_type_str = NULL;
    const char* actual_type_str = NULL;
    const char* room_id = NULL;
    const char* bed_id_str = NULL;
    ...
    if (g_ward_list != NULL && g_ward_list->next != NULL)
    {
        ...
        while (ward_curr != NULL)
        {
            ...
            if (ward_curr->ward_type == WARD_ICU)
            {
                icu_total++;
                if (!ward_curr->is_occupied)
                    icu_free++;
    ...
    occupancy_rate = (total_beds > 0) ? ((double)occupied_beds / total_beds * 100) : 0.0;

    printf("总床位数        : %d\n", total_beds);
    printf("已占用床位数    : %d\n", occupied_beds);
    printf("空闲床位数      : %d\n", free_beds);
    printf("床位占用率      : %.2f%%\n", occupancy_rate);
    printf("ICU 总数        : %d (空闲: %d)\n", icu_total, icu_free);
    printf("普通病房总数    : %d (空闲: %d)\n\n", general_total, general_free);

    // 床位预警判断
    has_bed_warning = 0;
    if (free_beds == 0)
    {
        printf(RED "[WARN] 当前无空闲床位！\n" RESET);
        has_bed_warning = 1;
    }
    if (occupancy_rate >= 80.0)
    {
        printf(RED "[WARN] 床位资源紧张！\n" RESET);
        has_bed_warning = 1;
    }
    if (icu_free == 0 && icu_total > 0)
    {
        printf(RED "[WARN] ICU 床位已满！\n" RESET);
        has_bed_warning = 1;
    }
    if (general_free == 0 && general_total > 0)
    {
        printf(RED "[WARN] 普通病房床位已满！\n" RESET);
        has_bed_warning = 1;
    }

    if (!has_bed_warning && total_beds > 0)
    {
        printf(GREEN "[OK] 当前床位资源充足。\n" RESET);
    }
    else if (total_beds == 0)
    {
        printf(GRAY "[INFO] 当前无床位数据。\n" RESET);
    }
    ...
    else
    {
        ...
        while (curr != NULL)
        {
            if (curr->is_active)
            {
                ...
                if (patient != NULL)
                {
                    ...
                    // 病房类型字符串映射
                    switch (curr->recommended_ward_type) {
                        case WARD_ICU: recommend_type_str = "ICU"; break;
                        case WARD_ISOLATION: recommend_type_str = "隔离病房"; break;
                        case WARD_SINGLE: recommend_type_str = "单人病房"; break;
                        default: recommend_type_str = "普通病房"; break;
                    }

                    if (strlen(curr->bed_id) > 0)
                    {
                        switch (curr->ward_type) {
                            case WARD_ICU: actual_type_str = "ICU"; break;
                            case WARD_ISOLATION: actual_type_str = "隔离病房"; break;
                            case WARD_SINGLE: actual_type_str = "单人病房"; break;
                            default: actual_type_str = "普通病房"; break;
                        }
                        actual_ward_type = actual_type_str;
                        ...
                        // 获取病房每日费用
                        switch (curr->ward_type) {
                            case WARD_ICU: daily_rate = 500.0; break;
                            case WARD_ISOLATION: daily_rate = 300.0; break;
                            case WARD_SINGLE: daily_rate = 400.0; break;
                            default: daily_rate = 120.0; break;
                        }
                    }

                    if (strlen(curr->bed_id) > 0 && curr->deposit_balance < daily_rate * 3)
                    {
                        warning_count++;
                        warn_msg = "押金不足";
                        show_warn = 1;
                    }
                    else if (strlen(curr->bed_id) == 0)
                    {
                        warning_threshold = 0.0;
                        switch (curr->recommended_ward_type) {
                            case WARD_ICU: warning_threshold = 500.0 * 3; break;
                            case WARD_ISOLATION: warning_threshold = 300.0 * 3; break;
                            case WARD_SINGLE: warning_threshold = 400.0 * 3; break;
                            default: warning_threshold = 120.0 * 3; break;
                        }
        ...
        if (warning_count == 0)
        {
            printf(GREEN "[OK] 当前无押金不足患者。\n" RESET);
        }
        else
        {
            printf(RED "总计 %d 位患者押金不足\n" RESET, warning_count);
        }
    }
...
static void handle_inpatient_query_by_id()
{
    ...
    while (1)
    {
        ...
        if (is_blank_string(query_str))
        {
            printf("[WARN] 住院号不能为空，请重新输入\n");
            continue;
        }
        if (strncmp(query_str, "I-", 2) != 0)
        {
            printf("[WARN] 住院号格式应为 I-xxx，请重新输入\n");
            continue;
        }
        break;
    }
...
static void handle_deposit_recharge()
{
    ...
    while (1)
    {
        ...
        // 转换为浮点数并验证（严格校验）
        if (!parse_double_strict(input_buf, &amount) || amount <= 0)
        {
            printf("[WARN] 充值金额必须大于0，请重新输入\n");
            continue;
        }
...
static void handle_transfer_bed()
{
    ...
    PatientNode* patient = NULL;
    InpatientRecord* inpatient = NULL;
    WardNode* old_bed = NULL;
    WardNode* new_target = NULL;
    const char* ward_type_str = NULL;
    ...
    while (1)
    {
        get_safe_string("请输入患者编号 (输入B返回，Q退出): ", patient_id, MAX_ID_LEN);
        ...
        // 检查患者是否已住院
        inpatient = find_active_inpatient_by_patient_id(patient_id);
        if (inpatient == NULL)
        {
            printf("[WARN] 未找到该患者当前住院中的记录，无法转床！\n");
            continue;
        }
        
        // 检查患者是否已分配床位
        if (is_blank_string(inpatient->bed_id))
        {
            printf("[WARN] 该患者未分配床位，不能进行转床操作\n");
            continue;
        }
        
        // 查找当前床位信息
        old_bed = find_bed_by_id(inpatient->bed_id);
        if (old_bed == NULL)
        {
            printf("[WARN] 患者当前床位信息异常，请联系管理员\n");
            continue;
        }
        
        // 保存原床位编号
        strcpy(old_bed_id, inpatient->bed_id);
        
        // 显示当前住院信息
        ward_type_str = old_bed->ward_type == WARD_ICU ? "ICU" :
            (old_bed->ward_type == WARD_ISOLATION ? "隔离病房" :
            (old_bed->ward_type == WARD_SINGLE ? "单人病房" : "普通病房"));
    ...
    while (1)
    {
        get_safe_string("\n请输入新床位编号 (输入B返回，Q退出): ", new_bed_id, MAX_ID_LEN);
        
        // 检查是否返回或退出
        if (my_strcasecmp(new_bed_id, "b") == 0)
        {
            printf("\n已返回上一级\n");
            system("pause");
            return;
        ...
        {
            printf("[WARN] 新床位编号不能为空，请重新输入\n");
            continue;
        }
        
        new_target = find_bed_by_id(new_bed_id);
        if (new_target == NULL)
        {
            printf("[WARN] 新床位不存在，请重新输入\n");
            continue;
        }
        if (strcmp(new_bed_id, old_bed_id) == 0)
        {
            printf("[WARN] 新床位与当前床位相同，无需转床，请重新输入\n");
            continue;
        }
        if (new_target->is_occupied)
        {
            printf("[WARN] 新床位已被占用，请重新输入\n");
            continue;
        }
