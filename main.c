...
static void handle_doctor_consultation()
{
    ...
    else if (decision == 4)
    {
        ...
        while (1)
        {
            estimated_days = get_safe_int("请输入预计住院天数 (输入0返回诊疗决策): ");
            if (estimated_days == 0)
            {
            ...
            printf("⚠️ 预计住院天数必须大于0！\n");
        }

        while (1)
        {
            deposit = get_safe_double("请输入初始押金金额 (输入0返回诊疗决策): ");
            if (deposit == 0.0)
            {
                patient->status = original_status;
                goto back_to_decision;
            ...
            {
                printf("⚠️ 当前余额不足缴纳住院押金，无法继续办理住院！\n");
                printf("   当前余额：%.2f 元，所需押金：%.2f 元\n", patient->balance, deposit);
                continue;
            }
            break;
        }
        ...
        while (1)
        {
            condition_level = get_safe_int("请输入病情级别 (1=普通, 2=重症, 输入0返回诊疗决策): ");
            ...
            printf("⚠️ 病情级别必须是 1 或 2！\n");
        }

        patient->balance -= deposit;
        if (!register_inpatient(patient->id, estimated_days, deposit, condition_level))
        {
            patient->balance += deposit;
            free_patient_ptr_list(waiting_list);
            system("pause");
        ...
        if (!consult_success)
        {
            InpatientRecord* inpatient_record = find_active_inpatient_by_patient_id(patient->id);
            if (inpatient_record != NULL)
            {
                if (inpatient_record->prev != NULL)
                {
                    inpatient_record->prev->next = inpatient_record->next;
                }
                if (inpatient_record->next != NULL)
                {
                    inpatient_record->next->prev = inpatient_record->prev;
                }
                free(inpatient_record);
            }
...
static void handle_patient_self_first_visit()
{
    ...
    while (1)
    {
        ...
        {
            case 0:
                get_safe_string("请输入您的身份证号: ", input_buffer, sizeof(input_buffer));
                ...
                {
                    strcpy(error_msg, "⚠️ 当前已是第一步，无法返回上一步。");
                    continue;
                }

                if (is_blank_string(input_buffer))
                {
                    strcpy(error_msg, "⚠️ 身份证号不能为空，请重新输入。");
                    continue;
                }

                if (!validate_id_card(input_buffer))
                {
                    strcpy(error_msg, "⚠️ 身份证号格式不合法，请重新输入。");
                    continue;
                }
...

                {
                    ...
                    if (existing_patient != NULL)
                    {
                        ...
                        if (existing_patient->is_blacklisted == 1 || existing_patient->is_blacklisted == 2)
                        {
                            printf("\n⚠️ 系统已识别到您的历史患者档案！\n");
                            printf("您的患者编号是：%s\n", existing_patient->id);
                            printf("您的姓名是：%s\n", existing_patient->name);
                            printf("当前状态：黑名单患者\n");
                            printf("提示：系统已同步您的黑名单状态。\n");
                            printf("本次将复用原患者档案继续后续业务处理，预约/挂号等操作将按黑名单规则限制。\n");
                            printf("\n请使用上述患者编号进行后续操作。\n");
                        }
                        else
                        {
            ...
            case 1:
                ...
                {
                    snprintf(error_msg, sizeof(error_msg), "⚠️ 患者姓名不能为空，请重新输入。");
                    continue;
                }

                {
                    int name_len = utf8_char_count(name);
                    if (name_len < 2 || name_len > 20)
                    {
                        snprintf(error_msg, sizeof(error_msg),
                                 "⚠️ 患者姓名长度必须在 2-20 个字符之间，请重新输入。");
                        continue;
                    }
                }
            ...
            case 2: // 年龄
            {
                get_safe_string("请输入您的年龄: ", input_buffer, sizeof(input_buffer));
                ...
                if (is_blank_string(input_buffer) || !is_digit_string(input_buffer))
                {
                    snprintf(error_msg, sizeof(error_msg), "⚠️ 年龄输入无效，请重新输入。");
                    continue;
                }

                int temp_age = atoi(input_buffer);
                if (temp_age < 0 || temp_age > 130)
                {
                    snprintf(error_msg, sizeof(error_msg), "⚠️ 年龄必须在 0-130 岁之间，请重新输入。");
                    continue;
                }
            ...
            case 3:
                get_safe_string("请输入您的性别（男/女）: ", input_buffer, sizeof(input_buffer));
                ...
                {
                    strcpy(error_msg, "⚠️ 性别输入无效，请输入 男 或 女。");
                    continue;
                }
            ...
            case 4:
                get_safe_string("请输入您的症状描述: ", input_buffer, sizeof(input_buffer));
                ...
                if (is_blank_string(input_buffer))
                {
                    strcpy(error_msg, "⚠️ 症状描述不能为空，请重新输入。");
                    continue;
                }
...
static void handle_patient_archive_update()
{
    ...
    input_balance:
    printf("当前余额：%.2f\n", patient->balance);
    get_safe_string("请输入新余额（输入 0 回退上一步，输入 00 退出；留空表示不修改）: ", temp_input, MAX_SYMPTOM_LEN);
    if (strcmp(temp_input, "00") == 0)
    {
        printf("操作取消！\n");
        return;
    ...
    if (strlen(temp_input) > 0)
    {
        ...
        // 校验是否为合法数字
        if (*endptr != '\0' || endptr == temp_input)
        {
            printf("⚠️ 余额输入无效，请输入合法金额。\n");
            goto input_balance;
        }
            ...
            case 3:
                {
                    ...
                    printf("\n================ 床位占用情况 ===============-\n");
                    printf("------------------------------------------------------\n");
                    printf("病房编号       床位编号       类型         患者编号       患者姓名       身份证号\n");
                    printf("------------------------------------------------------\n");
...
static void handle_bed_assign()
{
    ...
    // 输入患者编号（不能为空、必须存在、必须已住院但未分配床位；输入Q取消）
    while (1)
    {
        get_safe_string("请输入患者编号 (输入Q返回上一级): ", patient_id, MAX_ID_LEN);
    ...
    // 输入床位编号（不能为空、必须存在、必须空闲；输入Q取消）
    while (1)
    {
        get_safe_string("请输入床位编号 (输入Q返回上一级): ", bed_id, MAX_ID_LEN);
        ...
        // 检查床位是否存在且空闲
        if (!bed_is_available(bed_id))
        {
            printf("⚠️ 床位不存在或已被占用，请重新输入\n");
            continue;
        }
...
static void handle_transfer_bed()
{
    ...
    while (1)
    {
        ...
        // 检查新床位是否存在且空闲
        if (!bed_is_available(new_bed_id))
        {
            printf("⚠️ 新床位不存在或已被占用，请重新输入\n");
            continue;
        }
