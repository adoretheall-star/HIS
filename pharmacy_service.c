...
static void clear_patient_prescriptions(PatientNode* patient)
{
    PrescriptionNode* curr = NULL;
...
static void sync_latest_consult_record_to_completed(const char* patient_id)
{
    ConsultRecordNode* curr = NULL;
    ConsultRecordNode* latest = NULL;

    if (g_consult_record_list == NULL || is_blank_string(patient_id))
    {
        return;
    }
    ...
    if (latest != NULL)
    {
        latest->post_status = STATUS_COMPLETED;
    }
}
...
void show_paid_patients_waiting_for_dispense()
{
    ...
    while (curr != NULL)
    {
        if (curr->status == STATUS_WAIT_MED && curr->script_head != NULL && curr->script_count > 0)
        {
...
int dispense_medicine_for_patient(const char* patient_id)
{
    ...
    if (patient->status != STATUS_WAIT_MED)
    {
    ...
    while (curr_script != NULL)
    {
        ...
        if (med->stock == 0)
        {
            printf(RED "[WARN] 预警：药品 [%s] [%s] 当前库存为 0，已缺货，请及时补货。" RESET "\n", med->id, med->name);
        }
        else if (med->stock < LOW_STOCK_THRESHOLD)
        {
            printf(YELLOW "[WARN] 预警：药品 [%s] [%s] 当前库存为 %d，已进入低库存状态，请及时补货。" RESET "\n", med->id, med->name, med->stock);
        }
    ...
    clear_patient_prescriptions(patient);
    patient->status = STATUS_COMPLETED;
    sync_latest_consult_record_to_completed(patient->id);
    printf("提示：发药完成，处方已结转清空，患者就诊状态已更新为就诊结束。\n");
    return 1;
}
