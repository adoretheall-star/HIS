/*
 * 【代码分工说明】
 * 模块名称：患者服务模块 patient_service.c / patient_service.h
 * 主要负责人：周宇轩 55251328
 * 主要内容：
 * 1. 实现患者信息管理（新增、修改、查询、删除）和患者档案维护；
 * 2. 实现自助挂号、预约挂号、签到和退号功能；
 * 3. 实现黑名单管理、欠费管理和信用评级；
 * 4. 实现智能分诊、投诉管理和患者评价功能。
 * 参与说明：
 * 申贞隆 55251318 参与患者输入校验、身份证号脱敏和异常拦截；
 * 胡博畅 55251329 参与患者端界面排版和状态显示优化。
 */
...
/**
 * @brief 根据患者编号查找检查记录
 * @param head 检查记录链表头指针
 * @param patient_id 患者编号
 * @return 找到返回检查记录节点指针，未找到返回NULL
 */
CheckRecordNode* find_check_records_by_patient_id(CheckRecordNode* head, const char* patient_id)
{
    ...
    CheckRecordNode* curr = head->next; // 跳过头结点
    while (curr != NULL && curr != head) // 循环遍历链表
    {
        if (strcmp(curr->patient_id, patient_id) == 0) return curr; // 找到匹配的检查记录
        curr = curr->next;
    }
    return NULL; // 没有找到匹配的检查记录
}
...
/* 
 * 【功能作者】周宇轩 55251328 
 * 【功能说明】患者建档，首次来院登记，包含身份核验和智能分诊。
 * 【功能协助】申贞隆 55251318
 * 【协助说明】本功能涉及身份证号核验、敏感信息掩码输入、脱敏展示和 B/Q 返回防呆逻辑。
 */
/**
 * @brief 患者建档
 * @param name 患者姓名
...
/* 
 * 【功能作者】周宇轩 55251328 
 * 【功能说明】患者满意度评价，针对就诊服务进行星级评价和文字反馈。
 */
// ==========================================
// 患者满意度评价模块
// ==========================================
int submit_patient_evaluation(const char* patient_id)
{
    ...
    if (last_completed_record == NULL)
    {
        ...
        {
            printf("⚠️ 您的所有已完成就诊记录都已评价，不能重复评价！\n");
        }
        else
        {
...
/* 
 * 【功能作者】周宇轩 55251328 
 * 【功能说明】患者投诉提交，支持对医生、护士、药师的服务进行投诉。
 */
// ==========================================
// 患者投诉管理模块
// ==========================================
...
void show_patient_visit_timeline(const char* patient_id)
{
    ...
    {
        case STATUS_PENDING:
            printf("患者已建档，正在等待医生接诊\n");
            break;
        case STATUS_EXAMINING:
            printf("医生已接诊，患者正在进行检查\n");
            break;
        case STATUS_RECHECK_PENDING:
            printf("检查已完成，患者等待医生复诊\n");
            break;
        case STATUS_UNPAID:
            printf("医生已复诊，患者正在等待缴费\n");
            break;
        case STATUS_WAIT_MED:
            printf("缴费已完成，患者正在等待取药\n");
            break;
        case STATUS_NEED_HOSPITALIZE:
            printf("医生建议住院，患者正在办理住院登记\n");
            break;
        case STATUS_HOSPITALIZED:
            printf("患者已办理住院，正在住院治疗中\n");
            break;
        case STATUS_COMPLETED:
            printf("本次就诊流程已全部完成\n");
            break;
